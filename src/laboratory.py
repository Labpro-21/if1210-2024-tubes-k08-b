def laboratory(yourMonsData, coin, role):
  if role != 'admin' :
    ...
    while True: 
      ...
      inputMons = True                                                               # validasi input selain 1/2/3/dst/back
      for char in str(pilihMons) :
        if ord(char) < ord('0') or ord(char) > ord('9'):
          inputMons = False

      if pilihMons.lower() == 'back' :                                               # input = back
        print("Sampai bertemu lagi !")
        break

      if int(pilihMons) > len(yourMonsData) or int(pilihMons) < 1 :                  # input > jumlah monster atau input < 1
        inputMons = False
